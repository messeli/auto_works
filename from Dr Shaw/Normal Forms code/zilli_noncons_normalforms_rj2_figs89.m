...
epsln=  0.353;  % out of balance distance
f=m*epsln;
...
%campbell diag
Omvals=6.225;%3.8%:0.05:5.0;%3.37:0.0005:3.39;%2:0.01:2.5;%2.2:0.01:2.5;% 3.2:0.01:4.5;%3.7:0.01:3.9;   %0.2:0.2:6.4;%
...
for ii =1:(nguess*nOmvals)
    ...
    if abs(U(2))> 0.001 && abs(U(1))> 0.001 && solvecodes(ii)>0 
       ...
       fg=gmfigure;
        ...
        title(['Orbit $\Omega = ' sprintf('%.3f',Omega) '$'] , 'interpreter' , 'latex' , 'fontsize',fntsz);
      %  grid on
      xlim(1.2*[ -1 1])
      ylim(1.2*[ -1 1])
        axis square
        ...
        [~,resp ] = zillicontactsim(zmdl,Omega,[0 22*2*pi/omrs(ii)],y(:,1));
        subplot(1,2,1)
