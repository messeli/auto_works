...
!------------------------------------------------------------------------------
      ...
      SUBROUTINE FUNC(NDIM,U,ICP,PAR,IJAC,F,DFDU,DFDP) 
        ...
        F_n  = +0.5D0*(tanh(K*(R_2-1))+1) * (  KB_N*(R_2-DELT)  +  CB_N*(Q7*cos(CA) + Q8*sin(CA)) )
      ...
      SUBROUTINE STPNT(NDIM,U,PAR,T)  
        ...
        KB_N = 1D6  !look up formulas from s90 of b 1985Johnson Contact Mechanics Herzian contact ??
        CB_N = sqrt( (2*log10(REST)**2*M_2*KB_N)  /  (pi**2+log10(REST)**2)  ) 
        !PAR(11) reserved for period
        !PAR(12) reserved for torus sth ? 
        K = PAR(13)    !|tanh steppness
