...
tend = 1000 # 3000
tol = 1e-6 # 1e-7
Omeg_range = n.arange(0.1,7.2,0.1)  # ".01:0.01:7.01" 
print("Omeg_range length is {}".format(len(Omeg_range)) )

T = {} ; Q = {}
#|___CCC___|> LOOP ALL Omeg_range
#| PARFOR - 
# p = 1 ; nIdxLeft = len(Omeg_range) 
# while nIdxLeft > 0:
#| OR 
while True:
  #| PARFOR - Generate index batch for parfor
  # iRange = p:p+min(8,nIdxLeft)-1  
  # print('NEW INDEX BATCH '.format(iRange) )

  #| OR give all at the same time 
  iRange = range(0,len(Omeg_range))  
  ...
  for i in iRange:  # parfor
    ...
    Q.append(Q_new) 
    #|___SOL___|. 
  #| PARFOR -  Prepare next indexRange for parfor |Check if parfor now works without this indexRange
  # nIdxLeft = len(Omeg_range) - iRange[-1] 
  # p = iRange[-1] + 1  
  #| OR
  break
end_sim = time.time() 
...
#|___CCC___|. #|Entire Omeg_range finished.
